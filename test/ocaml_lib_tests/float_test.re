/* adapted from https://raw.githubusercontent.com/ocaml/ocaml/trunk/testsuite/tests/lib-float/test.ml */
/**************************************************************************/
/*                                                                        */
/*                                 OCaml                                  */
/*                                                                        */
...
let run = () => {
  let () = {
    ...
    assert(!Float.is_infinite(1.));
    assert(!Float.is_infinite(Float.nan));
    assert(Float.is_nan(Float.nan));
    assert(!Float.is_nan(1.));
    assert(!Float.is_nan(neg_infinity));
    assert(Float.is_integer(1.));
    assert(Float.is_integer(-1e10));
    assert(!Float.is_integer(1.5));
    assert(!Float.is_integer(Float.infinity));
    assert(!Float.is_integer(Float.nan));
    ...
    assert(
      {
        let x = 0x1.0000000000001p52; /* x + 0.5 rounds to x +. 1. */
        Float.round(x) == x;
      },
    );
    ...
    assert(
      Float.next_after(0x1.FFFFFFFFFFFFFp-2, 0.) == 0x1.FFFFFFFFFFFFEp-2,
    );
    assert(Float.(next_after(0x1.FFFFFFFFFFFFFp-2, infinity) == 0.5));
    assert(
      Float.(
        next_after(0x1.FFFFFFFFFFFFFp-2, neg_infinity) == 0x1.FFFFFFFFFFFFEp-2
      ),
    );
    ...
    assert(Float.sign_bit(-0.));
    assert(!Float.sign_bit(infinity));
